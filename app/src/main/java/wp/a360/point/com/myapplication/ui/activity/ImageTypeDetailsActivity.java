...
public class ImageTypeDetailsActivity extends BaseActivity implements XRefreshView.XRefreshViewListener{
    ...
    public void doBusiness(Context mContext) {
        initRefreshView();
        imageTypeID = getIntent().getIntExtra("imageTypeID", 0);
        imageTypeName = getIntent().getStringExtra("imageTypeName");
        imageTypeSize = getIntent().getIntExtra("imageTypeSize",0);
        details_top_name.setText(imageTypeName!=null?imageTypeName:searchKeyName+"");
        details_type_size.setText(imageTypeSize+"张");
        showLog("imageTypeID的结果:"+imageTypeID);
        typeData(imageTypeID);  //请求类型数据
    }
     ...
     * @param imageTypeID
     */
    private void typeData(int imageTypeID) {
            ...
            public void onResponse(String result) {
                try{
                    if (result != null) {
                        //请求成功时的回调
                        Gson gson = new Gson();
                        TypeToken<List<DailySelect>> tToken = new TypeToken<List<DailySelect>>() {
                        };
                        final List<DailySelect> listData = (List<DailySelect>) gson.fromJson(result, tToken.getType());
                        details_gridview.post(new Runnable() {
                            @Override
                            public void run() {
                                if(listData!=null){
                                    start = start+listData.size();
                                    tdAdapter.refresh(listData);
            ...
            switch (msg.what){
                case Constant.RESULT:
                    try{
                        ...
                        Gson gson = new Gson();
                        TypeToken<List<DailySelect>> tToken = new TypeToken<List<DailySelect>>() {};
                        List<DailySelect> data =  gson.fromJson(str, tToken.getType());
                        if(data!=null){
                            type = type+1;
                            start = start+data.size();//不为空，记录加一页数据
